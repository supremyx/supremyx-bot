...
async function generateWelcomeCard({ member, title, subtitle, color, accentColor }) {
  ...
  // Fond en rayures diagonales vert abstrait (même inclinaison que les rayures d'accent)
  const stripeColors = ['#0B3D1A', '#1A7A30', '#145C22', '#34C759'];
  const stripeW = 90;
  const slant = -170;
  ctx.save();
  ...
  // Assombrissement global des rayures (60%, éclairage augmenté de 100%)
  ...
  const subText = (subtitle || 'RALLIER • DOMINER • INSPIRER — BIENVENUE SUR SUPREMYX');
