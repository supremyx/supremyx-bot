  ...
  client.on('messageCreate', async message => {
    ...
    if (cmd === '!setrankreward') {
      ...
      if (isNaN(rank) || rank < 1 || !role)
        return message.reply('Usage : `!setrankreward <rang> @role [label]`\nExemple : `!setrankreward 1 @Champion 🥇 Champion`');
    ...
    if (cmd === '!rankrewards') {
      ...
      if (!rewards.length)
        return message.reply('Aucune récompense de rang configurée. Utilise `!setrankreward <rang> @role`.');
    ...
    // --- !delrankreward <rang> ---
    if (cmd === '!delrankreward') {
      if (!isStaff) return message.reply('Staff uniquement');

      const rank = parseInt(args[1]);
      if (isNaN(rank)) return message.reply('Usage : `!delrankreward <rang>`');
